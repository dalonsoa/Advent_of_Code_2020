...
fn get_passwords(args: &[String]) -> Vec<Password> {
    ...
    fs::read_to_string(filename)
        ...
        .split("\n")
        .map(|s| Password::from_str(s.parse::<String>().unwrap()))
        .collect()
}
...
fn main() {
    ...
    println!("Old policy valid passwords: {}/{}", valid_old, passwords.len());
    ...
    println!("New policy valid passwords: {}/{}", valid_new, passwords.len());
